...
#ifdef BSPFB
    // test for directly relying on BSP DMA
    static lv_disp_buf_t disp_buf_1;
    lv_disp_buf_init(&disp_buf_1, 0xc0000000, 0xc0000000, LV_HOR_RES_MAX * LV_VER_RES_MAX);   /*Initialize the display buffer*/
#else
    /* Example for 1) */
    static lv_disp_buf_t disp_buf_1;
...
static void disp_flush(lv_disp_drv_t * disp_drv, const lv_area_t * area, lv_color_t * color_p)
{
    ...
    int32_t x;
    int32_t y;
    uint32_t Address, width = 0;
//    Address = hltdc_discovery.LayerCfg[ActiveLayer].FBStartAdress + (((BSP_LCD_GetXSize()*Ypos) + Xpos)*(4));
    Address = 0xc0000000 + (((BSP_LCD_GetXSize()*area->y1) + area->x1)*(4));
    width = area->x2 - area->x1 +1;

    uint32_t InputColorMode = 0x00000000U;
...
#ifdef LVDMA
    	ll_raw_scan((uint32_t *)color_p, (uint32_t *)Address, width, InputColorMode);
